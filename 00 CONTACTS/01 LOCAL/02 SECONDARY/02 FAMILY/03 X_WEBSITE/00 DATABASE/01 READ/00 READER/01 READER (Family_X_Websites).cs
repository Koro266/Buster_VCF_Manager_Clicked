...
//LOCAL
using GROUP_XWEBSITE_ROW	= CONTACTS.LOCAL.SECONDARY.FAMILY.XWEBSITE.Row;
...
namespace CONTACTS.LOCAL.SECONDARY.FAMILY.XWEBSITE
{
	...
	{
		//___________________________________________________________________________________________________________________________________________
		public class FamilyWebsiteReader : BASE_READER
		{
			//___________________________________________________________________________________________________________________________________________
			public FamilyWebsiteReader( string sql_text ) : base( sql_text )
			{
			}
			//___________________________________________________________________________________________________________________________________________
			public FamilyWebsiteReader( string sql_text, OleDbParameter parameter ) : base( sql_text )
			{
				base.DbCommand.Parameters.Add( parameter );
			}
			//_______________________________________________________________________________________________________________________________________
			public GROUP_XWEBSITE_ROW ReadFamilyWebsite()
			{
			...
			//_______________________________________________________________________________________________________________________________________
			public Dictionary<int, BASE_ROW> ReadFamilyWebsites()
			{
				...
				try
				{
					...
					{
						group_website = ReadRow();
						group_websites.Add( group_website.PkFamily_X_Website.Value, group_website );
					}
			...
			private GROUP_XWEBSITE_ROW ReadRow()
			{
				...
				group_x_website.Append( new COLUMNS.PK_Family_X_Website		( base.GetPrimaryKey	( ORDINAL.PkFamily_X_Website )	) );
				group_x_website.Append( new COLUMNS.FK_Family				( base.GetForeignKey	( ORDINAL.FkFamily )				) );
